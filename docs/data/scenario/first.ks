...
雑な操作説明:矢印が表示されるぞ。すかさず対応する移動キー( h:←,  j:↓,  k:↑,  l:→) を入力しよう。Vimmiumは無効にしたほうがいいぞ。[l][cm]
[html top=0 left=0]
...
<div class="editor-block">メロスは激怒した。<br>必ず、かの邪智暴虐の王を除かなければならぬと決意した。<br><span class="editor-cursor">メ</span>ロスは計算機室でおにぎりを二十個食べた。<br>メロスには政治がわからぬ。</div>
[endhtml]
...
[eval exp="f.score += 2"]
[cm]
Vimちゃん「そう、それでいいの。行ごと消すなら素直に `dd`。分かっるじゃない」[l][r]
[jump target=*insert]
...
[eval exp="f.score += 2"]
[cm]
Vimちゃん「そう。それがいちばん早いわ。行末に足すなら、 `A` が適任ね」[l][r]
...
[eval exp="f.score -= 1"]
[cm]
Vimちゃん「‥‥マウスに頼るの？ そういうところ、あんまり好きじゃないわ」[l][r]
[jump target=*judgement]
...
[if exp="f.score <= 1"]
  [jump target=*bad_end]
[elsif exp="f.score >= 5 && f.only_hjkl_return == true && f.move_hits == 12"]
...
*hidden_end
[cm]
Vimちゃん「矢印キーにも逃げず素早いカーソル操作、ここまで来るなんて……大したものね」[l][r]
[html top=0 left=0]
<div data-choice-root="true"></div>
[endhtml]
...
いや違う。終了したのはゲームじゃなくて、おれの「これまでのエディタ人生」の方だ。[l][r]
気づけばおれは、Vim無しには生きられない身体になっていた[l][r]
【 HIDDEN END 】[l][cm]
[jump target=*start]
